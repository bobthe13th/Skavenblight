...
## Correction (post-deployment): the original Tasks 1/2 guard was too broad and caused a regression

**The user reported clanrats still stuck after this plan's original fix shipped.** Investigation (a diagnostic dump via `skavenblight_dumps/siege_dump_*.txt` plus `run/logs/latest.log`) found the original guard's premise was wrong: it required solid ground below *every* `BUILD_STAIR`/`BUILD_PILLAR`/`BUILD_SPIRAL` target, treating "no support yet" as inherently broken. But `TerrainEvaluator.isWalkableTerrain` short-circuits true on `isWalkableScaffold(foot)` alone — a placed stair/pillar/spiral is self-supporting for pathing purposes regardless of what's below it. A macro `SiegeProject` climbing through open space plans a whole chain of these steps in advance, where step N's support is step N-1 (built moments earlier) — "no support below yet" is the *normal* state for the next unbuilt step, not a bug. The original `isTargetStillValid()` change invalidated every such step on tick 1 (before `execute()` was ever reached), so the goal looped start→immediately-invalid→stop→restart forever with **no log output from either the give-up-after-stall path or the guard's own abort path** — which is exactly what made it look silently, totally stuck.

**Fix, corrected:** the guard now only fires when support was *already solid at the moment the target was claimed* and has since disappeared — the actual race this plan targets — never for a target that legitimately never had support to begin with. See `SiegeNode.SiegeAction.isClimbDependent()` and `AbstractSiegeConstructionGoal#supportSolidAtClaim` (captured in `start()`) for the mechanism. All references to "support below" in the tasks below describe the *original, over-broad* version; the corrected version is documented in the Verification Results section and in the code itself.
...
4. **Both fixes committed after user confirmation** (`8b49347` siege fix, `4226782` ChainLightningPayload fix, `f9017cb` plan doc), then merged with an incoming `Skavenblight/dev` merge (`f9190d6`) landing on the same branch from elsewhere — confirming concurrent activity on this checkout, consistent with the "docs" commit noted above.
5. **User reported clanrats still stuck after deploying the fix.** Diagnostic dump (`skavenblight_dumps/siege_dump_2026-07-29_15-53-26.txt`) showed a real macro `SiegeProject` staircase (15 `BUILD_STAIR` nodes, Y=-60 to Y=-44) with a large mob jam queued behind a single actively-building clanrat whose target's support (`pos.below()`) was genuinely air — by design, since it's the next unbuilt step in the chain. `run/logs/latest.log` had **zero** matches for either `"giving up on"` (the stall-give-up path) or `"aborted placement"` (the guard's own abort path) — proving the code was never even reaching those checkpoints, i.e. the bug was upstream of both: the original `isTargetStillValid()` change invalidated the target on tick 1 after every `start()`, before 15 ticks of animation could ever complete.
6. **Fix narrowed** (see "Correction" section at the top of this doc): added `SiegeAction.isClimbDependent()`, `AbstractSiegeConstructionGoal#supportSolidAtClaim` (snapshotted in `start()`), and changed the guard everywhere to require `supportSolidAtClaim == true` (support existed at claim time) before treating currently-missing support as a problem. Two call sites with no claim-time tracking (`StrandedGoal.java`, `SpiralSapperGoal.java` — neither implicated in the diagnosed bug) now explicitly pass `false`, preserving their original unguarded behavior rather than silently inheriting a check they were never designed around.
7. **Re-verified after narrowing:** `./gradlew compileJava` → `BUILD SUCCESSFUL`. `./gradlew runGameTestServer` → **`All 15 required tests passed :)`**, including `testWidenStairsGoalDoesNotPlaceFloatingStairWhenSupportRemovedMidAction` (still passes — it exercises support-present-then-removed, i.e. `supportSolidAtClaim == true`, which the narrowed guard still catches).

**Caveat that still applies:** the GameTest suite exercises the guard mechanism in isolation, not the exact multi-mob-jam macro-project scenario from the diagnostic dump. The user needs to re-test in their actual running world and, if clanrats are still stuck, send another diagnostic dump (`skavenblight:debug_flow_field_reader`, shift+right-click near an active Nexus) plus check `run/logs/latest.log` for the same two signatures — that combination is what actually resolved this round.
